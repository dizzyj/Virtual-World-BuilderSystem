...
namespace Mirror
{
    ...
    public static class NetworkServer
    {
        ...
        public static UnityEvent<NetworkConnection> addCon;
        ...
        static void Initialize()
        {
            ...
            addCon = new UnityEvent<NetworkConnection>();
        ...
        /// <summary>CUSTOM VW COUNSELING FUNCTION. Spawns NetworkIdentities in the specified scene on the server.</summary>
        // NetworkIdentity objects in a scene are disabled by default. Calling
        // SpawnObjects() causes these scene objects to be enabled and spawned.
        // It is like calling NetworkServer.Spawn() for each of them.
        public static bool SpawnObjectsInScene(Scene newScene)
        {
            // only if server active
            if (!active)
                return false;
            ...
            foreach (NetworkIdentity identity in identities)
            {
                // VW COUNSELING ADDED SCENE CHECK
                Scene identityScene = identity.gameObject.scene;
                if (identityScene.name == newScene.name) {
                    if (ValidateSceneObject(identity))
                    {
                        // Debug.Log($"SpawnObjects sceneId:{identity.sceneId:X} name:{identity.gameObject.name}");
                        identity.gameObject.SetActive(true);
